...
import com.example.android_rentalapp.todo.data.remote.ItemApi
...
class ItemRepository(private val itemDao: ItemDao) {
    ...
    suspend fun refresh(): Result<Boolean> {
        try {
            val items = ItemApi.service.find(Api.tokenInterceptor.token.toString())
            for (item in items) {
                itemDao.insert(item)
            }
    ...
    suspend fun update(item: Item): Result<Item> {
        try {
            val updatedItem = ItemApi.service.update(item, item.id)
